  ...
  try {
    ...
    if (messages) {
      ...
      if (type === "text" && messages.text?.body) {
        msgRaw = messages.text.body.trim().toLowerCase();
      } else if (type === "interactive") {
        if (messages.interactive.type === "button_reply") {
          msgRaw = messages.interactive.button_reply.id;
        } else if (messages.interactive.type === "list_reply") {
          msgRaw = messages.interactive.list_reply.id;
        }
      }

      // Step 0: Welcome message and greeting detection
      if (session.step === 0) {
        await sendTextMessage(from, "Yagya waste solutions welcome you on board!");

        if (
          msgRaw.includes("hi") ||
          msgRaw.includes("hello") ||
          msgRaw.includes("schedule") ||
          msgRaw.includes("pick up") ||
          msgRaw.includes("pickup")
        ) {
          await sendInteractiveList(
            from,
            "♻️ Yagya E-Waste Service",
      ...
      // Existing flow from step 1 onwards

      if (session.step === 1 && msgRaw) {
        session.data.category = msgRaw;
        await sendInteractiveButtons(from, "Choose your pickup vehicle:", [
          { id: "bike", title: "🏍️ Bike" },
          { id: "van", title: "🚐 Van" },
          { id: "truck", title: "🚛 Truck" },
        ...
        session.step = 2;
      } else if (session.step === 2 && msgRaw) {
        session.data.vehicle = msgRaw;
        await sendTextMessage(from, "Enter the approximate weight (in kg):");
        session.step = 3;
      } else if (session.step === 3 && type === "text") {
        session.data.weight = messages.text.body;
        await sendTextMessage(from, "Please enter your pickup address:");
        session.step = 4;
      } else if (session.step === 4 && type === "text") {
        session.data.address = messages.text.body;
        await sendTextMessage(from, "Enter preferred pickup date (DD-MM-YYYY):");
        session.step = 5;
      } else if (session.step === 5 && type === "text") {
        session.data.date = messages.text.body;
        await sendInteractiveButtons(from, "Select a pickup time slot:", [
          { id: "slot1", title: "10AM–12PM" },
          { id: "slot2", title: "12PM–4PM" },
        ...
        session.step = 6;
      } else if (session.step === 6 && msgRaw) {
        session.data.time = msgRaw;
        await sendInteractiveButtons(from, "Choose a payment method:", [
          { id: "upi", title: "💳 UPI" },
          { id: "cod", title: "💵 Cash on Pickup" },
        ]);
        session.step = 7;
      } else if (session.step === 7 && msgRaw) {
        session.data.payment = msgRaw;
        await sendTextMessage(from, "Please enter your contact number:");
        session.step = 8;
      } else if (session.step === 8 && type === "text") {
        session.data.contactNumber = messages.text.body;
        ...
        const readableTime = timeSlotsMap[session.data.time] || session.data.time;

        await sendTextMessage(
          from,
          `✅ Booking Confirmed!\n\n📌 Category: ${session.data.category}\n🚚 Vehicle: ${session.data.vehicle}\n⚖️ Weight: ${session.data.weight} kg\n🏠 Address: ${session.data.address}\n📅 Date: ${session.data.date}\n⏰ Time: ${readableTime}\n📞 Contact Number: ${session.data.contactNumber}\n💰 Payment: ${session.data.payment}\n\nThank you for using Yagya ♻️`
        );
